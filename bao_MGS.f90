    ...
    type, extends(TCosmoCalcLikelihood) :: BAO_MGS_Likelihood
    ...
    public BAO_MGS_Likelihood, BAO_MGS_Likelihood_Add
    contains

    subroutine BAO_MGS_Likelihood_Add(LikeList, Ini)
    class(TLikelihoodList) :: LikeList
    class(TSettingIni) :: ini
    Type(BAO_MGS_Likelihood), pointer :: this
    integer numbaosets, i

    if (Ini%Read_Logical('use_BAO_MGS',.false.)) then
        numbaosets = Ini%Read_Int('bao_MGS_numdatasets',0)
        if (numbaosets<1) call MpiStop('Use_BAO_MGS but numbaosets = 0')
        if (Ini%Haskey('BAO_fixed_rs')) then
            BAO_fixed_rs= Ini%Read_Double('BAO_fixed_rs',-1._mcp)
        end if
        do i= 1, numbaosets
            allocate(this)
            call this%ReadDatasetFile(Ini%ReadFileName(numcat('bao_MGS_dataset',i)))
            this%LikelihoodType = 'BAO_MGS'
            this%needs_background_functions = .true.
            call LikeList%Add(this)
        end do
        if (Feedback>1) write(*,*) 'read BAO_MGS data sets'
    end if

    end subroutine BAO_MGS_Likelihood_Add

    subroutine BAO_ReadIni(this, Ini)
    class(BAO_MGS_Likelihood) this
    class(TSettingIni) :: Ini
    character(LEN=:), allocatable :: bao_measurements_file, bao_invcov_file
    integer i,iopb
    Type(TTextFile) :: F

    if (Feedback > 0) write (*,*) 'reading BAO_MGS data set: '//trim(this%name)
    this%num_bao = Ini%Read_Int('num_bao_MGS',0)
    if (this%num_bao.eq.0) write(*,*) ' ERROR: parameter num_bao not set'
    this%type_bao = Ini%Read_Int('type_bao_MGS',1)
    if(this%type_bao /= 3 .and. this%type_bao /=2 .and. this%type_bao /=4 .and. this%type_bao /=5) then
        write(*,*) this%type_bao
        write(*,*)'ERROR: Invalid bao type specified in BAO_MGS dataset: '//trim(this%name)
        call MPIStop()
    end if
...
!this%Calculator%AngularDiameterDistance(zdr7)
    ...
    !print *, 'LS', zdr7, alphadr7, DVdr7, DAdr7, const_c, Hdr7, rsdrag_theory 
      if ((alphadr7.lt.0.8005).or.(alphadr7.gt.1.1985)) then
        ...
        BAO_DR11_loglike =  chi2/2.0
      endif


   if(feedback>1) write(*,*), 'MGS BAO likelihood = ', BAO_DR11_loglike
