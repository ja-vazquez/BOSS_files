    ...
    type, extends(TCosmoCalcLikelihood) :: BAO_Busca_Likelihood
    ...
    end type BAO_Busca_Likelihood

!JAV
    integer,parameter :: DR11_alpha_npoints=61
    ...
    public BAO_Busca_Likelihood, BAO_Busca_Likelihood_Add
    contains

    subroutine BAO_Busca_Likelihood_Add(LikeList, Ini)
    class(TLikelihoodList) :: LikeList
    class(TSettingIni) :: ini
    Type(BAO_Busca_Likelihood), pointer :: this
    integer numbaosets, i

    if (Ini%Read_Logical('use_BAO_Busca',.false.)) then
        numbaosets = Ini%Read_Int('bao_Busca_numdatasets',0)
        if (numbaosets<1) call MpiStop('Use_BAO_Busca but numbaosets = 0')
        if (Ini%Haskey('BAO_fixed_rs')) then
            BAO_fixed_rs= Ini%Read_Double('BAO_fixed_rs',-1._mcp)
        end if
        do i= 1, numbaosets
            allocate(this)
            call this%ReadDatasetFile(Ini%ReadFileName(numcat('bao_Busca_dataset',i)))
            this%LikelihoodType = 'BAO_Busca'
            this%needs_background_functions = .true.
            call LikeList%Add(this)
        end do
        if (Feedback>1) write(*,*) 'read BAO_Busca data sets'
    end if

    end subroutine BAO_Busca_Likelihood_Add

    subroutine BAO_ReadIni(this, Ini)
    class(BAO_Busca_Likelihood) this
    class(TSettingIni) :: Ini
    character(LEN=:), allocatable :: bao_measurements_file, bao_invcov_file
    integer i,iopb
    Type(TTextFile) :: F

    if (Feedback > 0) write (*,*) 'reading BAO_Busca data set: '//trim(this%name)
    !if (Feedback > 0) write (*,*) 'reading BAO_Busca data set:'//trim(this%prob) 
    this%num_bao = Ini%Read_Int('num_bao_Busca',0)
    if (this%num_bao.eq.0) write(*,*) ' ERROR: parameter num_bao not set'
    this%type_bao = Ini%Read_Int('type_bao_Busca',1)
    if(this%type_bao /= 3 .and. this%type_bao /=2 .and. this%type_bao /=4 .and. this%type_bao /=5) then
        write(*,*) this%type_bao
        write(*,*)'ERROR: Invalid bao type specified in BAO_Busca dataset: '//trim(this%name)
        call MPIStop()
    end if
   ...
   ! HS modified SDSS_dvtors to calculate D_A/rs 
    ...
    do while (ios.eq.0)
!JAV
        read (7,*,iostat=ios) tmp0,tmp1,tmp2,tmp3,tmp4
        if (ios .ne. 0) cycle
        nn = nn + 1
!JAV
!order is important
        ii = 1 + mod((nn-1),DR11_alpha_npoints)
        jj = 1 + ((nn-1)/DR11_alpha_npoints)
        DR11_alpha_perp_file(ii)   = tmp0
        DR11_alpha_plel_file(jj)   = tmp1

!JAV
        DR11_prob_file(ii,jj)      = tmp4/2.0 !prob got from chis2
...
!    real,parameter ::rd_fid=149.7482405038,H_fid=230.036275,DA_fid=1735.46077050    
    integer ii,jj
